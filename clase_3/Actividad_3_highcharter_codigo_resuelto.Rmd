...
### ¿Qué es *highcharter*?
Highcharter es un paquete de R para crear gráficos dinámicos de Highcharts. Highcharts es una biblioteca de gráficos en javascript.
...
*1. Serie 1 -> Personas empadronadas* con `hc_add_series(name='Personas empadronadas')`       
*2. No aparece la leyenda* con `hc_add_series(showInLegend=FALSE)`  
*3. Eje x: figuran los niveles de la variable categórica y el título 'Comuna'* con `hc_xAxis(categories = parse_number(tabla_departamento$departamento),title = list(text = "Comuna"))`  
*4. Eje y: figura el título 'Nro. de consultas'* con `hc_yAxis(title = list(text = "Nro. de consultas"))`  
*5. Se agrega un título* con `hc_title(text = "Cantidad de personas empadronadas por comuna",align ='left')`      
*6. Se agrega un subtítulo* `hc_subtitle(text = 'fuente: AGISE|UPE',align ='left')`    
*7. Se define un tema* con `hc_add_theme(highcharter::hc_theme_elementary())`

  2) ¿Por qué algunas opciones están escritas como listas (`list=(...)`)?
*Las opciones se escriben como listas cuando existen más opciones a definir para esa propiedad. Por ejemplo, para el título del eje y podemos pensar en subopciones como el tamaño de fuente, la orientación, etc. Básicamente, todas las opciones que figuren con {...} en la [Guía de referencia de Highcharts](https://api.highcharts.com/highcharts/) debemos escribirlas como listas.*
...
*Lo toma de `hc_chart(type = "line")` cuando ininicializamos el gráfico*

  4) ¿Cómo podemos cambiar el ancho de la linea de APS y HOSP y agregarles marcadores?
*hc_add_series(lineWidth=3,marker=list(enabled=FALSE,radius=2))*
...
*`crosshairs=TRUE` incorpora una linea vertical para un dado valor de x*
*`table=TRUE` controla el alineamiento de los valores del tooltip*
...
*El menú de exportación sólo incluye los valores que se muestran en ese momento en el gráfico. Al mismo tiempo, hay que controlar que los nombres de las series y variables sean los apropiados*
